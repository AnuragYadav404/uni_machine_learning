...
class Visualizer:
    ...
    def draw_weight_path(self,ax,w_hist,**kwargs):
        ...
        for j in range(len(w_hist)):  
            ...
            ax.scatter(w_val[0],w_val[1],s = 80, c = np.reshape(colorspec[j], (-1, 3)), edgecolor = self.edgecolor,linewidth = 2*math.sqrt((1/(float(j) + 1))),zorder = 3)
            ...
            if j > 0:
                ...
                if np.linalg.norm(pt1 - pt2) > head_length and arrows == True:
                    ...
                    w_old = pt1
                    w_new = pt2
                    ax.plot([w_old[0],w_new[0]],[w_old[1],w_new[1]],color = 'k',linewidth = 2,alpha = 1,zorder = 2)      # plot approx
